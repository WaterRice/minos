...
public class StringSourceCodeCompiler {
	...
	public static byte[] compile(String sourceCode, 
			DiagnosticCollector<JavaFileObject> compileCollector) throws Exception {
		...
		boolean result = javaCompiler.getTask(null, javaFileManager, compileCollector,
				null, null, Arrays.asList(javaFileObject)).call();
	...
	/*
	 * 实现自己的JavaFileObject类,为了:
	 * 1.动态编译的对象不是文件而是字符串
	 * 2.动态编译后的结果不是class文件,而是对应的字节码流
	 */
	private static class MyJavaFileObject extends SimpleJavaFileObject {
		...
		public MyJavaFileObject(URI uri, String source) {
