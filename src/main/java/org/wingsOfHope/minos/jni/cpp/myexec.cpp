...
void myexec(long used_time)
{
    ...
    */
    ...
    case LANG_CPP:
    {
        ...
        sprintf(tmp, "%s/main", exec_path);
        execl(tmp, tmp, (char *)NULL);
        break;
    }
    case LANG_JAVA:
    {
        execl("/root/env/jdk/jdk1.8.0_171/bin/java",
              "java",
              "-Xms128M",
              "-Xms512M",
    ...
    case LANG_PYTHON:
        char tmp[4096];
        memset(tmp, 0, sizeof(tmp));
        sprintf(tmp, "%s/main.py", exec_path);
        execl("/usr/bin/python", "python", tmp, (char *)NULL);
        break;
    case LANG_RUBY:
...
// 写文件
static void save(char *code, char *path)
{
...
int main(int argc, char **argv)
{
    ...
    char code[] = "import sys; a,b = map(int, sys.stdin.readline().split());print(\"%d\" % (a + b)); ";
    ...
    sprintf(work_dir, "/mnt/e/myProgram/c/demo/judger/jni");
    sprintf(testcase_dir, "%s/testcase/%ld", work_dir, question_id);
    sprintf(runtime_dir, "%s/runtime", work_dir);
    ...
    // 先写文件，再进行必要的编译
    char tmp[4096], cmd[1024];
    FILE *fp = NULL;
    memset(tmp, 0, sizeof(tmp));
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "mkdir %s", exec_path);
    system(cmd);
    switch (language)
    {
    case LANG_C:
    {
        sprintf(tmp, "%s/main.c", exec_path);
        fp = fopen(tmp, "w");
        fprintf(fp, "%s", code);
        fclose(fp);
        // 编译链接
        sprintf(cmd, "gcc %s -o %s/main -std=c11", tmp, exec_path);
        if (system(cmd) == -1)
        {
            judge_status = JUDGE_CE;
    ...
    case LANG_CPP:
        sprintf(tmp, "%s/main.cpp", exec_path);
        fp = fopen(tmp, "w");
        fprintf(fp, "%s", code);
        fclose(fp);
        // 编译链接
        sprintf(cmd, "g++ %s -o %s/main -std=c++11", tmp, exec_path);
        if (system(cmd) == -1)
        {
            judge_status = JUDGE_CE;
            printf("compile error");
        ...
        break;
    case LANG_JAVA:
        sprintf(tmp, "%s/Main.java", exec_path);
        fp = fopen(tmp, "w");
        fprintf(fp, "%s", code);
        fclose(fp);
        // 编译链接
        sprintf(cmd, "javac %s", tmp);
        if (system(cmd) == -1)
        {
            judge_status = JUDGE_CE;
            printf("compile error");
    ...
    while (judge_status == JUDGE_RJ && (dirp = readdir(dp)) != NULL)
    {
        ...
        if (prepare_test_case(test_count, dirp->d_name) == 0)
            continue;
    ...
    // 清理现场
    // 1. runtime/src/run_id
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "rm -r %s", exec_path);
    system(cmd);

    // 2. dst
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "rm %s/%ld-*", dst_dir, run_id);
    system(cmd);
